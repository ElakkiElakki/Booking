...
System.out.println("Total date spans found: "
+ allDates.size());
...
if (count < 5) {
System.out.println("Date on calendar: "
+ d.getAttribute("data-date")
+ " visible: " + d.isDisplayed());
count++;
}
}
...
System.out.println("Target date cells: "
+ dateCells.size());
...
return false;
    ...
    private void selectCheckoutDate(String checkinDate)
                                     throws InterruptedException {
        try {
            ...
            System.out.println("selectCheckoutDate: "
                + e.getMessage());
        }
    }

    // ── Navigate calendar to correct month ───────────────────────────────
    private void navigateCalendarToDate(String targetDate)
                                         throws InterruptedException {
        System.out.println("Navigating to: " + targetDate);

        for (int attempt = 0; attempt < 6; attempt++) {

            // ✅ Check if date visible
            ...
            // ✅ Print current month for debugging
            try {
                ...
                for (WebElement h : headers) {
                    ...
                    if (text.contains("2026") ||
                        text.contains("2027")) {
                        System.out.println("Calendar: " + text);
                    }
                }
    ...
    public void selectGuests(int adults, int children)
                             throws InterruptedException {
        try {
            // ✅ Click guests container to open popup
            guestsContainer.click();
            Thread.sleep(1000);
            System.out.println("Guests popup opened ✅");

            // ✅ Set adults count
            setGuestCount("Adults", adults);

            // ✅ Set children count
            setGuestCount("Children", children);

            // ✅ Click Done button
            try {
                guestsDoneBtn.click();
                System.out.println("Guests done ✅");
            } catch (Exception e) {
                ((JavascriptExecutor) driver)
            ...
            System.out.println("Guests set — Adults: " + adults
                + " Children: " + children + " ✅");
    ...
    // ── Set guest count ───────────────────────────────────────────────────
    private void setGuestCount(String type, int target)
                                throws InterruptedException {
        try {
            // ✅ Get current count
            WebElement countEl = driver.findElement(By.xpath(
                "//div[@data-testid='occupancy-popup']" +
                "//span[contains(text(),'" + type + "')]" +
                "/following-sibling::div" +
                "//span[@data-testid='occupancy-popup-" +
                type.toLowerCase() + "-value']"));

            int current = Integer.parseInt(
                countEl.getText().trim());
            ...
            if (target > current) {
                for (int i = 0; i < target - current; i++) {
                    driver.findElement(By.xpath(
                        "//button[@aria-label='Increase number of "
                        + type + "']")).click();
                    Thread.sleep(300);
                }
            }
            // ✅ Decrease if needed
                ...
                for (int i = 0; i < current - target; i++) {
                    driver.findElement(By.xpath(
                        "//button[@aria-label='Decrease number of "
                        + type + "']")).click();
                    Thread.sleep(300);
                }
            }
            ...
            System.out.println("setGuestCount " + type
                + ": " + e.getMessage());
        }
    }
    ...
    public boolean isResultsPageDisplayed() {
        try {
            new WebDriverWait(driver, Duration.ofSeconds(10))
                .until(ExpectedConditions.urlContains("searchresults"));
            return true;
        } catch (Exception e) {
            return false;
        }
